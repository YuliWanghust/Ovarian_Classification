...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        m_batchsize, _, height, width = x.size() #x[2,64,5,6]
        proj_query = self.query_conv(x)  #[2,8,5,6] #batch,channel,x,y
        ##0-batch; 1-channel; 2-x-height; 3-y-width   in query, key, value
        ...
        return self.gamma*(out_H + out_W) + x #[2,64,5,6]
        ...
        # proj_key = self.key_conv(x)  # [2,8,5,6,7]
